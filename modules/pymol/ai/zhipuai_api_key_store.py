...
ACCOUNT_NAME = "zhipuai_api_key"

_ENV_ZHIPUAI_KEY = "ZHIPUAI_API_KEY"
_ENV_KEY_SOURCE = "PYMOL_AI_ZHIPUAI_KEY_SOURCE"
_ENV_KEY_SOURCE_SAVED = "saved_keyring"

DEFAULT_ZHIPUAI_BASE_URL = "https://open.bigmodel.cn/api/coding/paas/v4"
DEFAULT_ZHIPUAI_MODEL = "GLM-5"
...
def _env_key() -> str:
    return str(os.getenv(_ENV_ZHIPUAI_KEY) or "").strip()
...
def load_saved_key_into_env_if_needed() -> ApiKeyStatus:
    ...
    os.environ[_ENV_ZHIPUAI_KEY] = saved_key
...
def clear_saved_key_and_loaded_env_if_needed() -> bool:
    ...
    current = str(os.getenv(_ENV_ZHIPUAI_KEY) or "").strip()
    source = str(os.getenv(_ENV_KEY_SOURCE) or "").strip()
    if (
        saved_key
        and current
        and source == _ENV_KEY_SOURCE_SAVED
        and current == saved_key
    ):
        os.environ.pop(_ENV_ZHIPUAI_KEY, None)
        os.environ.pop(_ENV_KEY_SOURCE, None)
        return True
    return False
...
def validate_key_live(
    key: str, model: str = DEFAULT_ZHIPUAI_MODEL, timeout_sec: float = 15.0
) -> None:
    ...
    model_id = str(model or DEFAULT_ZHIPUAI_MODEL).strip() or DEFAULT_ZHIPUAI_MODEL
    client = None
    ...
    try:
        ...
        base_url = (
            str(os.getenv("ZHIPUAI_BASE_URL") or DEFAULT_ZHIPUAI_BASE_URL).strip()
