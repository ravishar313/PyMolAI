...
from pymol.ai.zhipuai_api_key_store import (
    ApiKeyStoreError,
...
class AiZhipuAiApiKeyDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, *, on_changed=None):
        ...
        self.setWindowTitle("Z.AI Coding-Plan API Key")
        self.setModal(True)
        self.resize(520, 180)
        ...
        self.key_input.setPlaceholderText("Enter Z.AI API key")
    ...
    def _on_save(self):
        ...
        if not key:
            QtWidgets.QMessageBox.warning(
                self, "Save API Key", "Please enter a non-empty API key."
            )
            return
        ...
        os.environ["PYMOL_AI_ZHIPUAI_KEY_SOURCE"] = "saved_keyring"
        self.key_input.clear()
        self._notify_changed()
        self._refresh_status()
        QtWidgets.QMessageBox.information(
            self, "Save API Key", "API key saved to system keychain."
        )
    ...
    def _on_test(self):
        ...
        key = (
            str(self.key_input.text() or "").strip()
            or str(os.getenv("ZHIPUAI_API_KEY") or "").strip()
        )
        if not key:
            QtWidgets.QMessageBox.warning(
                self, "Test API Key", "No API key is available to test."
            )
            return
    ...
    def _on_test_finished(self, ok: bool, message: str):
        ...
        if ok:
            QtWidgets.QMessageBox.information(
                self, "Test API Key", "API key validation succeeded."
            )
            return
