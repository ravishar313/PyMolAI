...
from pymol.ai.api_key_store import (
    ApiKeyStoreError,
...
from pymol.ai.openrouter_client import DEFAULT_MODEL


class _KeyValidationWorker(QtCore.QObject):
    finished = QtCore.Signal(bool, str)

    def __init__(self, key: str, model: str, timeout_sec: float):
        super().__init__()
        self._key = str(key or "").strip()
        self._model = str(model or DEFAULT_MODEL).strip() or DEFAULT_MODEL
        self._timeout_sec = float(timeout_sec)

    @QtCore.Slot()
    def run(self):
        try:
            validate_key_live(self._key, self._model, timeout_sec=self._timeout_sec)
...
class AiApiKeyDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, *, model: str = DEFAULT_MODEL, on_changed=None):
        ...
        self.key_input.setPlaceholderText("Enter OpenRouter API key")
    ...
    def _on_save(self):
        ...
        os.environ["PYMOL_AI_OPENROUTER_KEY_SOURCE"] = "saved_keyring"
    ...
    def _on_test(self):
        ...
        key = str(self.key_input.text() or "").strip() or str(os.getenv("OPENROUTER_API_KEY") or "").strip()
        if not key:
            QtWidgets.QMessageBox.warning(self, "Test API Key", "No API key is available to test.")
            return

        self._set_testing_state(True)
        worker = _KeyValidationWorker(key, self._model, timeout_sec=10.0)
