...
class PyMOLQtGUI(QtWidgets.QMainWindow, pymol._gui.PyMOLDesktopGUI):
    ...
    def __init__(self):  # noqa
        ...
        self.resize(
            options.win_x
            + (220 if options.internal_gui else 0)
            + (340 if options.external_gui else 0),
            options.win_y + 18,
        )
        ...
        self.ai_zhipuai_api_key_action = ai_menu.addAction(
        ...
        for model_id, friendly_name in model_menu_entries():
            ...
            action.toggled.connect(
                lambda checked, m=model_id: checked and self._on_ai_model_selected(m)
            )
        ...
        self.ai_openbio_api_key_action.triggered.connect(
            self._open_ai_openbio_api_key_dialog
        )
        self.ai_zhipuai_api_key_action.triggered.connect(
            self._open_ai_zhipuai_api_key_dialog
        )
        self.ai_mode_work_action.toggled.connect(
            lambda checked: checked and self.set_ai_agent_mode("work")
        )
        self.ai_mode_tutor_action.toggled.connect(
            lambda checked: checked and self.set_ai_agent_mode("tutor")
        )
    ...
    def render_dialog(self, widget=None):
        ...
        form.input_units.currentTextChanged.connect(
            lambda s: form.input_height_units.setSuffix(s)
        )
        form.input_units.currentTextChanged.connect(
            lambda s: form.input_width_units.setSuffix(s)
        )
    ...
    def _sync_ai_settings_menu_from_runtime(self):
        ...
        if hasattr(self, "ai_mode_work_action") and hasattr(
            self, "ai_mode_tutor_action"
        ):
    ...
    def _open_ai_zhipuai_api_key_dialog(self):
        from .ai_zhipuai_api_key_dialog import AiZhipuAiApiKeyDialog

        self.ai_zhipuai_api_key_dialog = AiZhipuAiApiKeyDialog(
            self,
            on_changed=self._on_ai_api_key_changed,
        )
        self.ai_zhipuai_api_key_dialog.exec_()
    ...
    def _on_history_chat_selected(self, chat_id: str):
        ...
        if self._has_unsaved_chat_work():
            ...
            discard_btn = box.addButton(
                "Discard and continue", QtWidgets.QMessageBox.DestructiveRole
            )
        ...
        if not payload:
            QtWidgets.QMessageBox.warning(
                self, "Load Chat", "Could not load selected chat."
            )
            return
        ...
        if not session_loaded:
            fallback = self._chat_store.get_last_valid_session_path(
        ...
        self._chat_has_user_input = any(
            str((e or {}).get("role") or "") == "user"
            for e in events
    ...
    def _on_chat_new_requested(self):
        if self._chat_store.count_chats() >= self._chat_store.soft_cap:
            ...
            box.setText(
                "You have many saved chats. Choose a cleanup action or keep all."
            )
            keep_btn = box.addButton("Keep all", QtWidgets.QMessageBox.AcceptRole)
            del10_btn = box.addButton(
                "Delete oldest 10", QtWidgets.QMessageBox.DestructiveRole
            )
            del25_btn = box.addButton(
                "Delete oldest 25", QtWidgets.QMessageBox.DestructiveRole
            )
            del50_btn = box.addButton(
                "Delete oldest 50", QtWidgets.QMessageBox.DestructiveRole
            )
            manager_btn = box.addButton(
                "Open manager", QtWidgets.QMessageBox.ActionRole
            )
            cancel_btn = box.addButton("Cancel", QtWidgets.QMessageBox.RejectRole)
            box.exec_()
            clicked = box.clickedButton()
        ...
        keep_scene_btn = box.addButton(
            "Keep current scene, clear chat", QtWidgets.QMessageBox.AcceptRole
        )
        reset_scene_btn = box.addButton(
            "Clear chat + reinitialize scene", QtWidgets.QMessageBox.DestructiveRole
        )
        cancel_btn = box.addButton("Cancel", QtWidgets.QMessageBox.RejectRole)
        box.exec_()
        clicked = box.clickedButton()
    ...
    def _open_history_manager(self):
        dlg = ChatHistoryManagerDialog(
            self._list_chat_rows, self._delete_chat_by_id, self
        )
        dlg.exec_()
